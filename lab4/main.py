...
mylib = cdll.LoadLibrary('./cy.cpython-310-darwin.so')
...
def lab4_btn():
    ...
    #         self.hour = tk.Spinbox(self, from_=0, to=23, wrap=True, validate='focusout',
    #                                validatecommand=(self.reg, '%P'), invalidcommand=self.hour_invalid,
    #                                textvariable=self.hourstr, width=2)
    ...
    #                               validatecommand=(self.reg2, '%P'), invalidcommand=self.min_invalid,
    #                               textvariable=self.minstr, width=2)
    #         self.reg3 = self.register(self.sec_valid)
    #         self.secstr = tk.StringVar(self, '30')
    #         self.sec = tk.Spinbox(self, from_=0, to=59, wrap=True, validate='focusout',
    #                               validatecommand=(self.reg3, '%P'), invalidcommand=self.sec_invalid,
    #                               textvariable=self.secstr, width=2)
    #         self.hour.grid()
    ...
    #         return valid
    #
    #     def sec_invalid(self):
    #         self.secstr.set('30')
    #
    #     def sec_valid(self, input):
    #         if (input.isdigit() and int(input) in range(60) and len(input) in range(1, 3)):
    #             valid = True
    #         else:
    #             valid = False
    #         if not valid:
    #             self.sec.after_idle(lambda: self.sec.config(validate='focusout'))
    #         return valid
    ...
    def dop_string_modify():
        ...
        num = 0
        hexadecimal = str(''.join([random.choice('ABCDEF0123456789') for i in range(6)]))
        textWidget.tag_configure(hexadecimal, foreground="seagreen1")

        def date_finder(date, tag):
            count = tk.IntVar()
            num = 0
            while True:
                index = textWidget.search(date, "matchEnd", "searchLimit", count=count, regexp=False)
                if index == "": break
                if count.get() == 0: break  # degenerate pattern which matches zero-length strings

                textWidget.insert("%s+%sc" % (index, count.get()), name)
                textWidget.mark_set("matchStart", index)
                textWidget.mark_set("matchEnd", "%s+%sc" % (index, count.get()))
        ...
        num += date_finder("07.12.2022", hexadecimal)
        num += date_finder("08.12.2022", hexadecimal)
        num += date_finder("09.12.2022", hexadecimal)
        num += date_finder("10.12.2022", hexadecimal)
    ...
    file_btn = tk.Button(window_last, text="Открыть файл", width=10, command=lambda: open_file())
    file_btn.grid(row=2, column=0)
    find_btn = tk.Button(window_last, text="Найти включения", width=10, command=lambda: find_sig())
    find_btn.grid(row=2, column=1, sticky=tk.W)
    dop_btn = tk.Button(window_last, text="Доп", width=10, command=lambda: dop_string_modify())
    dop_btn.grid(row=2, column=1, sticky=tk.E)
