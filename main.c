...
/*unsigned int devision(unsigned int digit, unsigned int shift) {
    return digit >> shift;
}
...
int main() {
    char c = '\n';
    int tmp_digit = 0;
    if (scanf("%d%c", &tmp_digit, &c) != 2 || c != '\n' || tmp_digit < 0) {
        printf("n/a\n");
        return -1;
    }
    register unsigned int digit = tmp_digit;
    for (int i = 1; i <= 4; i++) {
        printf("%d >> %d = %d / 2^%d = %d / %d = ",
               digit, i, digit, i, digit, (int)pow(2, i));
        if (pow(2, i) > digit) {
            printf("data loss\n");
            break;
        }
        digit = devision(digit, i);
        printf("%d\n", digit);
    }
    // Чтобы программа не завершалась сразу, а ждала нажатия // какой-либо клавиши
    //system("pause");
    return 0;
}*/
...
static unsigned char Df_Key[24] = {
    0x01,0x23,0x25,0x67,0x89,0xab,0xcd,0xef,
...
static unsigned short bigbyte[8] = {
...
short edf;
{
    ...
    for (i = 0; i < 16; i++) {
        ...
        for (j = 0; j < 28; j++) {
            l = j + totrot[i];
            if (l < 28) pcr[j] = pc1m[l];//возможно ошибка, не поняла l или единица
...
register unsigned long *block, *keys;
{
    ...
    right ^= work;
    right ^= (work << 16);
    work = ((leftt >> 8) ^ leftt) & 0x33333333L;
    leftt ^= work;
    right ^= (work << 8);
...
void main(void) {
    ...
    for (i = 0; i < 10; i+= 2) printf("Block %0ld = %081x %081x.\n", i / 2, data[i], data[i + 1]);
